...
- (void)viewDidLoad {
    ...
    [_apis addObjectsFromArray:@[@"镜像翻转接口测试"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([indexPath row] == 0)
    {
        ...
        hud.labelText = @"正在获取用户设备列表...";
        [EZOPENSDK getDeviceList:0
                        pageSize:10
                      completion:^(NSArray *deviceList, NSInteger totalCount, NSError *error) {
                          if (error)
                          {
                              hud.labelText = [NSString stringWithFormat:@"设备列表获取失败（%d）", (int)error.code];
                          ...
                          for (EZDeviceInfo *deviceInfo in deviceList)
                          {
                              if (deviceInfo.cameraNum > 0 && deviceInfo.status == 1) {
                                  ...
                                  hud.labelText = @"正在操作设备通道画面翻转...";
                                  [EZOPENSDK controlVideoFlip:deviceSerial
                                                     cameraNo:cameraInfo.cameraNo
                                                      command:EZDisplayCommandCenter
                                                       result:^(NSError *error) {
                                                           hud.labelText = [NSString stringWithFormat:@"画面翻转结果：error code is %d", (int)error.code];
                                                           [hud hide:YES afterDelay:0.5];
                                                       }];
                                  break;
                              }
