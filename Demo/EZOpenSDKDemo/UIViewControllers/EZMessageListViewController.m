...
- (void)viewDidLoad {
    ...
    [EZOPENSDK getUnreadMessageCount:self.deviceInfo.deviceSerial messageType:EZMessageTypeAlarm completion:^(NSInteger count, NSError *error) {
        self.title = [NSString stringWithFormat:@"消息（%d）", (int)count];
    }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView.allowsMultipleSelectionDuringEditing)
    {
        ...
        {
            self.deleteSelected.title = [NSString stringWithFormat:@"删除(%d)",(int)[self.selectedMessageArray count]];
            self.deleteSelected.enabled = YES;
            self.readAll.title = [NSString stringWithFormat:@"标记已读(%d)",(int)[self.selectedMessageArray count]];
...
- (NSString *)dateStringWithUserDefine:(NSString *)dateString
{
    ...
    if ([date isThisYear])
    {
        ...
        {
            return [NSString stringWithFormat:@"%ld月%ld日 %@", (long)[date month], (long)[date day], weekDays[[date weekday]-1]];
        }
    }
    else
    {
